...
Tunnel d'estimation de `/estimation` : un wizard de questions en langage client, un moteur de calcul déterministe alimenté par les grilles tarifaires officielles, et une capture de lead (email, téléphone, entreprise) pour rappel commercial.
...
engine.ts ── computeEstimate : (réponses + grilles) -> fourchettes par service
        ...
        ▼
ResultScreen ── affichage + LeadCapture -> emails.ts (lead + notification équipe)
```
...
| `src/app/api/estimation/*` | Persistance (Prisma `Estimate`), analyse IA, capture de lead + envoi email |
...
3. **On n'affiche pas de chiffre qu'on ne sait pas tenir.** Volume au-delà du dernier palier, Cloud & PRA : carte "sur devis" (`kind: "sur-devis"`), exclue des totaux.
...
Pas de socle fixe. Si aucun palier ne couvre le volume (ex. 55 postes, dernier palier 40), le service bascule en `kind: "sur-devis"` avec la ligne "Au-delà de 40 postes, sur devis".
...
4. Mode B : profil régulier/plein (`RENFORT_MODE_B_PROFILS`) + `renfort-duree` répondue -> décote -5 % (6 mois) ou -10 % (12 mois), plancher `low × 0.85`
5. × jours/mois du profil (`renfort-profil` : ponctuel 2, régulier 10, plein 20), ± `spreadPercent` de la grille autour du tarif jour
6. Agrégé dans le total mensuel (`kind: "recurring"`)

### `sur-devis` : toujours sur devis

Ex. Cloud & PRA. Aucun calcul : la grille porte la ligne explicative (`pricing.line`), la carte affiche un CTA rendez-vous. Exclu des totaux `monthlyMin/Max` et `oneShotMin/Max`.
