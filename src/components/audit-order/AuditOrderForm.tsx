...
const labelClass =
  "mb-2 block text-[11px] font-bold uppercase tracking-widest text-white";
...
export function AuditOrderForm({
  ...
  initialValues?: AuditOrderFormInitialValues;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      tva: tva.trim(),
      consent,
      ...(initialValues?.seoAuditId
        ? { seoAuditId: initialValues.seoAuditId }
        : {}),
    };
    ...
    try {
      ...
      if (!checkoutRes.ok) {
        ...
        throw new Error(
          json?.error ?? "La redirection vers le paiement a échoué.",
        );
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label htmlFor="order-tva" className={labelClass}>
          ...
          <span className="font-normal text-white/70">(optionnel)</span>
        </label>
        <Input
          id="order-tva"
          type="text"
      ...
      <p className="text-xs text-white/70">Paiement sécurisé via Stripe</p>    </form>
  );
}
