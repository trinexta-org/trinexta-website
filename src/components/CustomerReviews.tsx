...
import { BlobDotsBackground } from "@/components/BlobDotsBackground"
...
function ReviewCard({ review }: { review: Review }) {
  ...
  return (
    ...
    >
      <div className="bg-white border border-border hover:border-secondary/40 hover:shadow-md transition-all duration-300 rounded-2xl flex flex-col gap-4 p-6 h-full shadow-sm relative">
...
export function CustomerReviews() {
  ...
  return (
    <Section id="avis-clients" className="relative py-20 overflow-hidden">
  <BlobDotsBackground />
  <Container className="relative z-10">
        <div className="lg:hidden">
          ...
          >
            {reviews.map((review) => (
              <div key={review.id} className="snap-center shrink-0 w-[85vw] md:w-87.5 h-auto">
          ...
          <div className="flex items-center justify-center gap-4 mt-6 relative z-40">
            <button onClick={handlePrev} className="w-12 h-12 rounded-full bg-white border border-border flex items-center justify-center text-primary hover:bg-accent hover:border-secondary/30 transition-all active:scale-95" aria-label="Avis précédent">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <button onClick={handleNext} className="w-12 h-12 rounded-full bg-white border border-border flex items-center justify-center text-secondary-strong hover:bg-secondary hover:text-white hover:border-secondary transition-all active:scale-95" aria-label="Avis suivant">
              <ChevronRight className="w-5 h-5" />
            </button>
          </div>
        </div>

        <div className="hidden lg:grid grid-cols-2 gap-8 h-150 overflow-hidden relative group max-w-4xl mx-auto">

          <div className="absolute inset-x-0 top-0 h-24 bg-linear-to-b from-surface to-transparent z-10 pointer-events-none" />
          <div className="absolute inset-x-0 bottom-0 h-24 bg-linear-to-t from-surface to-transparent z-10 pointer-events-none" />
        ...
        <div className="mt-12 flex justify-center relative z-20">
          <a
            ...
            className="group relative inline-flex items-center justify-center overflow-hidden rounded-full bg-primary px-8 py-4 shadow-lg transition-all duration-500 border border-primary/10"
          >
